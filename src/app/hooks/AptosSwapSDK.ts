...
type Reserves = { reserveX: bigint; reserveY: bigint; pairAddress: string; tsMs: number; stale: boolean };
...
export class AptosSwapSDK {
    ...
    // FA views
    private async viewPrimaryBalance(owner: string, metadataAddr: string): Promise<bigint> {
    ...
    async getReserves(xMeta: string, yMeta: string, opts?: { refresh?: boolean; refreshIfStale?: boolean }) {
...
// --- quoting ---
    ...
    // --- tx builders ---
    buildRegisterPoolTx(xMeta: string, yMeta: string, sender: string): InputTransactionData {
        return {
            ...(sender ? { sender } : {}),
            data: {
    ...
    async buildAddLiquidityTx(
        ...
        computed: { optimal: { x: bigint; y: bigint }; xMin: bigint; yMin: bigint; pairAddress: string };
    }> {
        ...
        const optA = this.getOptimalAdd(xDesA, yDesA, reserves.reserveX, reserves.reserveY);
        const slip = slippageBps ?? this.defaultSlippageBps;
    ...
    async buildRemoveLiquidityTx(
        ...
        payload: InputTransactionData;
        computed: { expected: { xOut: bigint; yOut: bigint }; minX: bigint; minY: bigint; pairAddress: string; lpMeta: string };
    }> {
        const { reserves } = await this.getReserves(xMeta, yMeta, { refresh: true });
    ...
    async buildSwapTx(
        ...
        sender: string,
        slippageBps?: number,
    ): Promise<{ payload: InputTransactionData; computed: { expectedOut: bigint; minOut: bigint } }> {
