...
import { WarningTwoIcon, CheckCircleIcon, InfoOutlineIcon, InfoIcon } from "@chakra-ui/icons";
...
export function AgentOwnerTwitterActions({ agent, onDraftCreated, apiBase: apiBaseProp }: Props) {
    ...
    const isOwner = !!ownerWallet && !!currentWallet && ownerWallet === currentWallet;
    ...
    const [composerDraftId, setComposerDraftId] = React.useState<string | null>(null);
    ...
    const refreshKeys = React.useCallback(async () => {
        ...
        setKeysLoading(true);
        try {
            const res = await fetch(`${apiBase}/agents/${agent.fa_id}/twitter/keys/status`, {
                headers: { ...authHeader() },
            });
    ...
    const refreshUsage = React.useCallback(async () => {
        ...
        try {
            ...
            const res = await fetch(`${apiBase}/agents/${agent.fa_id}/twitter/usage`, {
                headers: { ...authHeader() },
            });
            const body = await res.json().catch(() => ({}));
            const next = {
                        ...
                        <ChakraTooltip.Positioner>
                            <ChakraTooltip.Content>
                                <Text fontSize="sm" whiteSpace="pre-line">
                                    {
                                        keys?.configured ? `X API connected to @${keys?.user?.username}\n` : 'X API keys invalid/not set\n'
                                    }
                                    {usageLoading || !usage
                                        ? "Loading quota…"
                                        : `Posts today: ${usage.sent_today}/${usage.daily_cap}\nRemaining posts: ${
                                            usage.remaining
