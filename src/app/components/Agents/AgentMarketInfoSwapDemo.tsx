...
export function AptosSwapDemo() {
    console.log("AptosSwapDemo");
    const { wallet, account, isConnected } = useAptosWallet();
    console.log({ wallet, account, isConnected });

    // Fullnode URL that works with @aptos-labs/ts-sdk (include /v1)
    const nodeUrl = "https://ultra-withered-patina.aptos-mainnet.quiknode.pro/804be4e05ef290503e6020df7efd44fb2ad52b8c/v1";
    ...
    const config = new AptosConfig({ network: Network.MAINNET, fullnode: nodeUrl });
    ...
    const iconUri = "https://mir-s3-cdn-cf.behance.net/project_modules/hd/690b4f103757219.5f54032af3b55.png"

    // Demo-sized amounts (raw u64 as bigint)
    const amountIn = 15_000_000n; // for swap
    ...
    useEffect(() => {
        ...
        (async () => {
            console.log("[DEMO] Wallet connected:", account.address);
                // try {
                //     const {payload} = await swapSDK.buildCreateAgentTx(name, symbol, iconUri, account.address.toString());
            ...
            // 1) CREATE POOL
            // try {
            //     const payload = swapSDK.buildRegisterPoolTx(xMeta, yMeta, account.address.toString());
            //     console.log(payload, wallet);
            //     const res = await swapSDK.submitWithWallet(wallet, payload);
            ...
            // // 6) REMOVE LIQUIDITY
            // try {
            //     const lpBalance = await swapSDK.getWalletLPBalance(account.address.toString(), xMeta, yMeta);
