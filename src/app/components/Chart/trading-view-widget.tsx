...
const DEFAULT_CHART_OPTIONS: Omit<ChartingLibraryWidgetOptions, "container" | "datafeed"> = {
	...
	time_frames: [
		{ text: "1H", resolution: "1" as ResolutionString, description: "1 Hour", title: "1H" },
			...
			return {
				format: (price, signPositive) => {
					...
					if (preciseValue < 1 && preciseValue > 0) {
						const orderOfMagnitude = Math.floor(Math.log10(Math.abs(preciseValue)));
...
export const TradingViewWidget = ({
	...
	useEffect(() => {
		...
		const datafeed = new PriceDataFeed(token, queryClient, onPriceUpdate) as any;
		...
		chart.current.onChartReady(() => {
			...
			try {
				// Get the chart object and scroll to the rightmost position
				...
				if (chartWidget) {
					...
					setTimeout(() => {
						...
						console.log("📊 Set visible range from", new Date(fourHoursAgo * 1000), "to", new Date((now + 600) * 1000));
