...
function useAptUsd() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const r = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/apt-price`);
...
export default function TokenSwapForm({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (amt > bal) {
        setBalanceError(
          `Insufficient balance (${fromAtomic(bal, dec)} ${symbol})`
        );
      } else {
        setBalanceError("");
      }
  ...
  useEffect(() => {
    ...
    const off6 = swapSDK.emitter.on("reserves:update", ({ entry }: any) => {
      ...
      setReserves(
        entry ? { reserveX: entry.reserveX, reserveY: entry.reserveY } : null
      );
    });
    return () => {
      off1?.();
  ...
  useEffect(() => {
    refreshBalances();
  }, [isConnected, account?.address, agentMeta]);

  /* ----------------------------- reserves/quote --------------------------- */
  const doReserves = useCallback(async () => {
    if (!agentMeta || !swapSDK) return;
    try {
      const { reserves } = await swapSDK.getReserves(APT_META, agentMeta, {
        refreshIfStale: true,
      });
      ...
      if (tab === "buy") {
        const spotHuman =
          rAG === 0n
            ? Infinity
            : (Number(rAPT) / Number(rAG)) *
              Math.pow(10, agentDecimals - APT_DECIMALS);
        setSpotPrice(spotHuman);
      } else {
        const spotHuman =
          rAPT === 0n
            ? Infinity
            : (Number(rAG) / Number(rAPT)) *
              Math.pow(10, APT_DECIMALS - agentDecimals);
        setSpotPrice(spotHuman);
      }
    ...
    try {
      ...
      if (side === "pay") {
        ...
        if (inAmt <= 0n) {
          ...
          try {
            const q = await swapSDK.quoteExactIn(
              inMeta,
              outMeta,
              one,
              true,
            ...
            setReserves({
              reserveX: q.reserves.reserveX,
              reserveY: q.reserves.reserveY,
            });
        ...
        if (outAmt <= 0n) {
          ...
          try {
            const q = await swapSDK.quoteExactOut(
              inMeta,
              outMeta,
              oneOut,
              true,
  ...
  const isUserCancel = (e: any) => {
    ...
    return (
      code === 4001 || // common "user rejected" code
  ...
  const onExecute = async () => {
    ...
    try {
      ...
      if (isUserCancel(e)) {
        toaster.create({
          type: "info",
          description: "Swap cancelled in wallet",
          duration: 2500,
        });
      } else {
        toaster.create({
  ...
  const receiveLabel =
    tab === "buy" ? `You receive (${agentSymbol})` : "You receive (APT)";
  const buttonLabel =
    tab === "buy" ? `Buy ${agentSymbol}` : `Sell ${agentSymbol} for APT`;
  ...
  const reserveAPT = reserves
    ? aptFirst
      ? reserves.reserveX
      : reserves.reserveY
    : 0n;
  const reserveAG = reserves
    ? aptFirst
      ? reserves.reserveY
      : reserves.reserveX
    : 0n;
  ...
  return (
    <Box borderRadius={16} bg={colorTokens.blackCustom.a3} p={4} flexShrink={0}>
      ...
      <NumberInput.Root
        ...
        value={pay}
        onValueChange={(d: any) =>
          onPayChange(d?.valueAsString ?? d?.value ?? "")
        } // FIX
      >
        <InputGroup
          endElement={
      ...
      <NumberInput.Root
        ...
        value={receive}
        onValueChange={(d: any) =>
          onReceiveChange(d?.valueAsString ?? d?.value ?? "")
        } // FIX
      >
        <InputGroup
          endElement={
      ...
      <Text mt={3} color={colorTokens.gray.timberwolf} fontSize={13}>
        ...
        {tab === "buy"
          ? `1 APT ≈ ${unitOut ?? "—"} ${agentSymbol} (~$${formatThousands(
              aptUsd?.toFixed(2) ?? ""
            )})`
          : `1 ${agentSymbol} ≈ ${unitOut ?? "—"} APT (~$${formatThousands(
              (aptUsd * parseFloat(unitOut ?? "0"))?.toFixed(2) ?? ""
            )})`}
        {quoting && <Spinner size="sm" ml={2} />}
      </Text>
      ...
      <Box mt={3}>
        {renderTokenInfoField(
          ...
          <>
            {" "}
            {isFiniteNum(spotPrice)
              ? formatTinyPrice(spotPrice!.toString())
              : "—"}{" "}
            {tab === "buy" ? `APT` : `${agentSymbol}`}
          </>
        )}
        {isFiniteNum(executionPrice) &&
          renderTokenInfoField(
            ...
            <>
              {" "}
              {isFiniteNum(executionPrice)
                ? formatTinyPrice(executionPrice!.toString())
                : "—"}{" "}
              {tab === "buy" ? `APT` : `${agentSymbol}`}
            </>
          )}
        ...
        {renderTokenInfoField(
          ...
          <>
            {" "}
            {reserves
              ? `${fromAtomic(reserveAPT, APT_DECIMALS)} APT / ${fromAtomic(
                  reserveAG,
                  agentDecimals
                )} ${agentSymbol}`
              : "—"}
